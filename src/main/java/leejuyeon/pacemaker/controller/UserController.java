...
public class UserController {
    ...
    public ResponseEntity<String> updateUserInfo(@AuthenticationPrincipal User user,
                                                 @RequestBody @Valid UserDto.Info info) {
        return ResponseEntity.ok(userService.updateUserInfo(user, info));
    }

    // 테스트
    @GetMapping
    public ResponseEntity<String> show(@AuthenticationPrincipal User user) {
